...
- `frontend/` — **SvelteKit 2 SPA** (adapter-static → `dist/`, `ssr`/`prerender` off) + **Tailwind v4** + **Skeleton v5**.
  ...
  - Shell: `src/routes/+layout.svelte` (Skeleton Navigation rail/sidebar via `Navigation`; Toast.Group; dialog hosts). No `src/views/` layer: each `+page.svelte` is a self-contained page (reads its own params/query). Components used by a SINGLE route are colocated next to that route (e.g. `src/routes/control/acl/AclPanel.svelte`, `src/routes/operations/*`); only genuinely shared components live in `src/components/` (SchemaForm/Editor, control_plane/BootstrapForm+format.ts, obtainium list/modals used by several routes) or `src/lib/components/` (dialogs). Rule: don't put single-use "panels" in components/.
