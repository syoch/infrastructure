...
class BackupManager:
    ...
    def serialize_db(cls, session) -> dict:
        ...
        for ext in cls._get_extensions():
            try:
                ...
                logger.error(f"Error during backup serialization for extension '{ext.__class__.__name__}': {e}")
        return {
            "extensions": ext_data
        }
        ...
        if strategy == "overwrite":
            ...
            for table in reversed(Base.metadata.sorted_tables):
                try:
                    ...
                    logger.warning(f"Failed to clear table '{table.name}': {table_err}")
            session.flush()
        ...
        for ext in cls._get_extensions():
            ...
            if ext_name in extensions_data:
                try:
                    ...
                    logger.error(f"Error restoring database data for extension '{ext_name}': {e}")
                    raise e
        session.flush()
                    ...
                    try:
                        ...
                        for subfolder, local_path in dirs.items():
                            if os.path.exists(local_path):
                                ...
                                logger.info(f"Bundled assets directory '{local_path}' into '{subfolder}'")
                                packed_dirs.append(subfolder)
                    except Exception as e:
                        logger.error(f"Error backing up physical directories for extension '{ext.__class__.__name__}': {e}")
    ...
    def restore_backup_tarball(cls, in_path: str, session, storage_ext=None, strategy: str = "overwrite"):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for ext in cls._get_extensions():
                try:
                    ...
                    logger.warning(f"Failed to restore directories for extension '{ext.__class__.__name__}': {e}")
            ...
            try:
                ...
                if "extensions" not in db_data:
                    logger.info("Adapting v1.0 flat database backup to v2.0 modular format...")
                ...
                session.commit()
                logger.info("Restoration database changes committed successfully.")
            except Exception as e:
                session.rollback()
                logger.error(f"Restoration database changes rolled back due to error: {e}")
                raise e
        ...
        logger.info("Restoration process finished.")
