...
# Find the repository root (where backend/manage.py lives)
portal_locate_repo() {
    local dir="$OI_ROOT"
    while [[ "$dir" != "/" ]]; do
        if [[ -d "$dir/backend" ]] && [[ -f "$dir/backend/manage.py" ]]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
    done
    die "Could not locate infrastructure repo root (looked for backend/manage.py upward from $OI_ROOT)"
}
...
portal_restore_backup() {
    ...
    ( cd "$repo" && python3 backend/manage.py --config "$cfg" restore --in "$tarball" ) \
...
portal_start_server() {
    ...
    (
        cd "$repo"
        exec python3 backend/app.py --config "$cfg"
    ) >>"$OI_TMP_DIR/portal.log" 2>&1 &
